...
class Calibrator:
    ...
    def run(self):
        ...
        if args.show:
            self.show_result(
                self.imgs, self.imgpoints, self.K, self.dist, self.rvec, self.tvecs
            )
    ...
    def show_result(self, imgs, imgpoints, mtx, dist, rvecs, tvecs):
        ...
        for i, (img, rvec, tvec, imgpoint) in enumerate(
            zip(imgs, rvecs, tvecs, imgpoints)
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument("input", help="input video for calibration")
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--w", type=int, default=6, help="the width of inner corners of chessboard"
    )
    parser.add_argument(
        "--h", type=int, default=8, help="the height of inner corners of chessboard"
    )
    parser.add_argument(
        "--show",
        action="store_true",
